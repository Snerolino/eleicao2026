-- Cria, de forma restrita e idempotente, proposição/versão factual de veto ALRS.
...
begin
  ...
  loop
    ...
    values ('ALRS — Portal da Transparência — veto nominal', 'oficial', item->>'source_url', item->>'title', item->>'source_sha256')
    ...
    if v_prop_id is null then
      ...
      values (v_external_id, 'alrs', 'outro', (item->>'number')::integer, (item->>'year')::integer, item->>'title', item->>'source_url') returning id into v_prop_id;
    end if;
    ...
    if v_version_id is null then
      ...
      values (v_prop_id, v_version_key, 'Veto total — votação nominal', 'alrs:' || md5(item->>'title'), v_source_id, coalesce((item->>'effective_from')::timestamptz, now())) returning id into v_version_id;
    end if;
