...
-- 1. Adicionar coluna slug (única, não nula)
...
-- 4. Tornar coluna obrigatória (já é not null, mas confirmar constraint)
alter table candidates
  alter column slug set not null;

-- 5. Índice para lookups por slug
create index if not exists idx_candidates_slug on candidates(slug);

-- 6. Constraint de formato (apenas alfanumérico e underscore)
alter table candidates
