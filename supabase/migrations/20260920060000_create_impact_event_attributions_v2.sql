...
-- Editors may draft and refine attribution content, but they cannot directly
-- approve/reject it or delete history. Review-state transitions are RPC-gated.
grant insert on table public.impact_event_attributions to authenticated;
revoke update, delete on table public.impact_event_attributions from authenticated;
grant update (
  object_voted_kind,
  ...
  rationale,
  updated_at
) on public.impact_event_attributions to authenticated;
