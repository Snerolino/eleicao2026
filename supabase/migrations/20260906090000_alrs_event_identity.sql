-- Separa eventos ALRS com mesma versão/data, mas matérias/objetos distintos.
-- Mantém compatibilidade com rows antigos que não carregam event_identity.
...
begin
  ...
  for item in select value from jsonb_array_elements(p_rows) loop
    ...
    event_identity := nullif(item->>'event_identity','');
    ...
    event_external_id := 'alrs-nominal-' || md5(coalesce(event_identity, version_id::text || '|' || to_char(vote_date at time zone 'UTC','YYYY-MM-DD')));
    ...
    if exists(select 1 from legislative_votes lv where lv.voting_event_id=event_id and lv.candidate_id=candidate) then
      if exists(select 1 from legislative_votes lv where lv.voting_event_id=event_id and lv.candidate_id=candidate and lv.value=vote_value) then existing_count := existing_count + 1;
