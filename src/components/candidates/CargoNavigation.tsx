...
export function CargoNavigation({ positions, counts, selected, savedCount, onSelect }: CargoNavigationProps) {
  ...
  return (
    <nav aria-label="Navegação por cargo" className="flex gap-2 overflow-x-auto border-y border-[var(--color-border-editorial)] py-3 [scrollbar-width:thin]">
      <button type="button" aria-current={selected === '' ? 'page' : undefined} onClick={() => onSelect('')} className={`min-h-11 shrink-0 rounded-sm border px-3 font-mono text-xs uppercase tracking-wider ${selected === '' ? 'border-[var(--color-institutional)] bg-[color-mix(in_srgb,var(--color-institutional)_10%,var(--color-paper))] text-[var(--color-institutional)]' : 'border-[var(--color-border-editorial)] text-[var(--color-muted-ink)]'}`}>Todos</button>
      {items.map((position) => (
        <button key={position} type="button" aria-current={selected === position ? 'page' : undefined} onClick={() => onSelect(position)} className={`min-h-11 shrink-0 rounded-sm border px-3 font-mono text-xs uppercase tracking-wider ${selected === position ? 'border-[var(--color-institutional)] bg-[color-mix(in_srgb,var(--color-institutional)_10%,var(--color-paper))] text-[var(--color-institutional)]' : 'border-[var(--color-border-editorial)] text-[var(--color-muted-ink)]'}`}>
          {POSITION_LABEL[position]} · {counts.get(position) ?? 0}
        </button>
      ))}
      <button type="button" aria-current={selected === 'saved' ? 'page' : undefined} onClick={() => onSelect('saved')} className={`min-h-11 shrink-0 rounded-sm border px-3 font-mono text-xs uppercase tracking-wider ${selected === 'saved' ? 'border-[var(--color-institutional)] bg-[color-mix(in_srgb,var(--color-institutional)_10%,var(--color-paper))] text-[var(--color-institutional)]' : 'border-[var(--color-border-editorial)] text-[var(--color-muted-ink)]'}`}>★ Salvos · {savedCount}</button>
    </nav>
  );
}
