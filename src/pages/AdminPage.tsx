                ...
                return (
                  <Fragment key={item.proposition_version_id}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 font-mono text-xs uppercase tracking-wider">
                      {(() => {
                        const safePropPage = sanitizeUrl(item.proposition_page);
                        return safePropPage ? (
                          <a href={safePropPage} target="_blank" rel="noreferrer" className="text-[var(--color-institutional)] underline-offset-4 hover:underline">
                            Fonte da proposição
                          </a>
                        ) : (
                          <span className="text-gray-400">Fonte da proposição</span>
                        );
                      })()}
                      {item.source_urls.slice(0, 2).map((sourceUrl) => {
                        const safeSourceUrl = sanitizeUrl(sourceUrl);
                        return safeSourceUrl ? (
                          <a key={sourceUrl} href={safeSourceUrl} target="_blank" rel="noreferrer" className="text-[var(--color-institutional)] underline-offset-4 hover:underline">
                            Fonte do voto
                          </a>
                        ) : (
                          <span key={sourceUrl} className="text-gray-400">Fonte do voto</span>
                        );
                      })}
                    </div>
