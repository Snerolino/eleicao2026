...
export function ComparePage() {
  ...
  // Optimization: Stable toggle callback using functional state updater
  // to prevent O(N) re-renders in the CandidateListItem grid.
  const toggleCandidate = useCallback((id: string) => {
    setSearchParams((prev) => {
      const prevIds = parseSharedCandidateIds(prev.get('candidatos'), validCandidateIds);
      const next = prevIds.includes(id)
        ? prevIds.filter((selectedId) => selectedId !== id)
        : [...prevIds, id].slice(0, 4);
