...
describe('ComparePage H5.3', () => {
  it('preserva parâmetros compartilháveis enquanto os candidatos carregam', () => {
    ...
    const urlNode = screen.getByLabelText(/url atual/i);
    expect(urlNode.textContent?.replace(/%2C/g, ',')).toContain('/comparar?candidatos=tse-1,tse-2');
  ...
  it('limita rota compartilhável a 4 candidaturas válidas', () => {
    ...
    const urlNode = screen.getByLabelText(/url atual/i);
    expect(urlNode.textContent?.replace(/%2C/g, ',')).toContain('/comparar?candidatos=tse-1,tse-2,tse-3');
  });
