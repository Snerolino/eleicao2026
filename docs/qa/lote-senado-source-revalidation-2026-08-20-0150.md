# QA — Revalidação de fontes nominais do Senado (2026-08-20 01:50 UTC)
...
- Coincidência de bytes contra o manifesto: **5/6**.
...
O manifesto versionado continua divergente do conteúdo atualmente servido pelas URLs oficiais. Cinco respostas mantiveram o tamanho esperado; todas as seis possuem SHA-256 diferente. Nenhum voto, fonte, manifesto, snapshot público ou registro remoto foi alterado. O item permanece **fail-closed**.
...
- Worktree limpa antes deste relatório; alteração limitada a esta documentação e ao checkpoint operacional.
...
- Commit documental: `9a7053090cc305d0b2f96b400656695c8e1d1bd3`, publicado em `origin/main`.
- Backup Cloudflare `334951434`, run `32322524502`, concluiu `completed/success` com `headSha` idêntico.
- Produção raiz: HTTP 200.
- `https://rs.votopraquem.org/release.json`: HTTP 200; release `9a70530-20260820T015156137Z`, versão `0.2.484`, `snapshot.row_count=1003` e SHA do run confirmado; `commit_sha` no payload veio nulo.
