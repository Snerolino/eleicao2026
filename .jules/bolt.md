...
## 2026-09-07 - [O(N log N) Sorting in React Components]
**Learning:** Using chained `.sort()` with `.indexOf()` for ordering subsets against a canonical array has O(N log N * N) complexity and can cause silent data drops if dynamic values missing from the canonical array sort to -1.
**Action:** Replace this pattern with an O(N) single-pass filter over the canonical array (`canonical.filter(item => subset.has(item))`), ensuring to append any unknown items from the subset to avoid data loss.
