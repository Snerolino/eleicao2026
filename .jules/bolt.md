...
## $(date +%Y-%m-%d) - [Defer Expensive Computations]
**Learning:** Filtering large candidate lists synchronously on every keystroke can cause input lag.
**Action:** Use `useDeferredValue` for the search query to decouple input state updates from expensive filtering operations, keeping typing responsive.
