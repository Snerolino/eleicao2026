...
const items=(merit.items??[]).filter(x=>p0Ids.has(x.proposition_version_id)||p1Ids.has(x.proposition_version_id)).map(x=>({...x,official_merit_confirmed:true,event_type:'merit_confirmed',review_gate:'official_event_confirmed',substantive_source_gate:p0Ids.has(x.proposition_version_id)?'green':'blocked_until_impact_sources',factual_source_gate:'green',review_status:'pending_review',editorial_status:'pending_review',human_review_required:true,remote_apply:false}));
