...
MODEL="${OPENCODE_AGENT_MODEL:-openai/gpt-5.6-luna}"
...
ISOLATED_SNAPSHOT="$(mktemp -d "${TMPDIR:-/tmp}/eleicao2026-opencode.XXXXXX")"
trap 'rm -rf -- "$ISOLATED_SNAPSHOT"' EXIT
cp -a "$SNAPSHOT"/. "$ISOLATED_SNAPSHOT"/
cd "$ISOLATED_SNAPSHOT"
