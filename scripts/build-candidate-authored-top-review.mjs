...
const projects=[...groups.values()].sort((a,b)=>b.candidate_tse_ids.length-a.candidate_tse_ids.length||a.id.localeCompare(b.id)).slice(0,25).map(p=>({...p,candidate_occurrences:p.candidate_tse_ids.length}));
const out='/tmp/camara-authored-unique-review-25.json';await writeFile(out,`${JSON.stringify({schema_version:'1.0.0',packet_type:'candidate_authored_projects_unique_editorial_batch',mode:'pending_review',remote_apply:false,content_read:false,selection:'unique projects sorted by exact candidate coverage',projects},null,2)}\n`);console.log(JSON.stringify({projects:projects.length,candidate_occurrences:projects.reduce((n,p)=>n+p.candidate_occurrences,0),unique_candidates:new Set(projects.flatMap(p=>p.candidate_tse_ids)).size,output:out}));
