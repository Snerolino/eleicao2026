...
function localState() {
  ...
  const inventory = readJson('artifacts/reprocess-impact-v2/release-inventory.json');
  const pilot = readJson('data/legislative-import/alrs/alrs-attribution-pilot-v1.json');
  return {
    ...
    release_inventory_count: Number(inventory.inventory_count ?? 0),
    release_inventory_counts: inventory.counts ?? {},
    attribution_pilot_mode: pilot.mode ?? 'unknown',
    attribution_pilot_selected_count: Number(pilot.selected_count ?? 0),
  };
}
