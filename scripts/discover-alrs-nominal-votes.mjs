...
function decode(value) {
  return value.replace(/&quot;|&#34;|&#x22;/gi, '"').replace(/&apos;|&#39;|&#x27;/gi, "'").replace(/&lt;|&#60;|&#x3c;/gi, '<').replace(/&gt;|&#62;|&#x3e;/gi, '>').replace(/&amp;|&#38;|&#x26;/gi, '&').replace(/&#x([0-9a-f]+);/gi, (_, hex) => String.fromCodePoint(parseInt(hex, 16))).replace(/&#([0-9]+);/g, (_, dec) => String.fromCodePoint(Number(dec)));
}
function dataItems(html) {
  return [...html.matchAll(/\bdata-item(?![\w:-])\s*=\s*(["'])([\s\S]*?)\1/gi)].flatMap((match) => {
    try { return [JSON.parse(decode(match[2]))]; } catch { return []; }
  });
}
...
function candidatesByName() {
  ...
  for (const row of rows) {
    const name = normalize(row.full_name ?? row.nome_urna ?? row.ballot_name ?? row.name);
    if (!name) continue;
    const list = map.get(name) ?? [];
    list.push({ candidate_id: row.id, tse_candidate_id: row.tse_candidate_id, name });
    map.set(name, list);
  }
  return map;
}
async function get(url) {
  ...
  return { url, http_status: response.status, bytes: Buffer.byteLength(body), sha256: createHash('sha256').update(body).digest('hex'), items: response.ok ? dataItems(body) : [] };
}
const main = await get(base);
if (main.http_status !== 200) throw new Error(`ALRS principal HTTP ${main.http_status}`);
const parliamentarians = [...main.items, ...[...main.items].map(() => null)].length ? [...main.items] : [];
...
const pages = [];
for (const parliamentarian of catalog) {
  for (const year of years) {
    ...
    pages.push({ solicitante_id: parliamentarian.solicitante_id, name: parliamentarian.name, year, ...source, data_item_count: source.items.length, candidate_match_count: parliamentarian.exact_candidate_matches.length, items: source.items.map((item) => ({ tipoProjeto: item.tipoProjeto, numProposicao: item.numProposicao, anoProposicao: item.anoProposicao, dataVotacao: item.dataVotacao, voto: item.voto, resultadoVotacao: item.resultadoVotacao, materia: item.materia })) });
  }
}
