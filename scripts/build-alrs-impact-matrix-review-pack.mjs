...
const items = merit.items.map((item) => ({
  ...
  source_gate: item.source_urls.every((url) => sourceByUrl.get(url)?.ok === true) ? 'green' : 'blocked',
...
const pack = { schema_version: '1.0.0', packet_type: 'alrs_impact_matrix_review_pack', unit_of_work: 'one_matrix_per_proposition_version', review_status: 'pending_review', remote_apply: false, public_approval: false, totals: { versions: items.length, source_gate_green: items.filter((item) => item.source_gate === 'green').length, factual_votes: items.reduce((sum, item) => sum + item.factual_vote_count, 0) }, items };
